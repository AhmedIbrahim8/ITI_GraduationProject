...
void TIMER_EnableInterrupt(TIMER_IDType Timer_Number,
		TIMER_InterryptType InterruptID) {
	switch(Timer_Number)
	{
	case TIMER2_SELECTION:
		switch (InterruptID) {
...
void TIMER_DisableInterrupt(TIMER_IDType Timer_Number,
		TIMER_InterryptType InterruptID) {
	switch(Timer_Number)
	{
	case TIMER2_SELECTION:
...
//void TIMER_voidHWICUConfig(void)
//{
//
////	 /* GPIO Configuration */
...
//
//	/* CC1 channel is configured as input, IC1 is mapped on TI1 */
//	  TIMER5->CCMR1 &= ~CCMR1_CC1S;
//	  TIMER5->CCMR1 |= CCMR1_CC1S_0;
//
//	  /* No input capture 1 prescaler */
//	  /* No Input capture 1 filter */
//
//	  /* Select input polarity for TI1FP1 active on rising edge */
//	  TIMER5->CCER &= ~CCER_CC1P;
//	  TIMER5->CCER &= ~CCER_CC1NP;
//
//	  /* Timer 1 input channel 1 IC2 configuration */
//	  /* CC2 channel is configured as input, IC2 is mapped on TI1 */
//	  TIMER5->CCMR1 &= ~CCMR1_CC2S;
//	  TIMER5->CCMR1 |= CCMR1_CC2S_1;
//
//	  /* Select input polarity for TI1FP2 active on falling edge */
//	  TIMER5->CCER |= CCER_CC2P;
//	  TIMER5->CCER &= ~CCER_CC1NP;
//
//	  /* Timer 1 trigger configuration */
//	    /* Select Filtered Timer Input 1 (TI1FP1) trigger */
//	  TIMER5->SMCR &= ~SMCR_TS;
//	  TIMER5->SMCR |= (SMCR_TS_0 | SMCR_TS_2);
//
//	    /* Select reset mode */
//	  TIMER5->SMCR &= ~SMCR_SMS;
//	  TIMER5->SMCR |= SMCR_SMS_2;
//
//	  /* Enable Interrupt */
//	  TIMER5->DIER |= DIER_INT;
//
//}
